...
import { COST_PLANNING_ARTICLES_DATA } from '../../data/knowledgeHub';
import { 
  DollarSign, 
  ArrowRight, 
  Clock, 
...
interface CostPlanningListPageProps {
  onNavigate: (route: string, slug?: string) => void;
  onOpenQuote: (type?: string) => void;
}

export const CostPlanningListPage: React.FC<CostPlanningListPageProps> = ({ 
  onNavigate, 
  onOpenQuote 
}) => {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredArticles = COST_PLANNING_ARTICLES_DATA.filter(a => 
    ...
    <div className="pt-28 md:pt-36 pb-20 space-y-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <Breadcrumbs 
        items={[
          ...
          { label: 'Cost & Planning', active: true }
        ]} 
      ...
      <section className="space-y-6 text-left border-b border-sky-100 pb-10">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-sky-50 border border-sky-200 text-sky-700 text-xs font-mono uppercase font-bold tracking-widest rounded">
            ...
            Knowledge Hub • 10 Planning &amp; Cost Guides
          </div>
          <button
          ...
          >
            <span>Explore Structural Comparisons</span>
            <ArrowRight className="w-4 h-4" />
          </button>
        </div>

        <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-slate-900 font-sans uppercase tracking-tight">
          PEB Cost, Budgeting &amp; Project Planning
        </h1>

        <p className="text-sm sm:text-base text-slate-600 leading-relaxed max-w-4xl">
          Responsible, engineering-backed guidance on the structural, material, and site factors that govern pre-engineered building investments. Understand steel tonnage, clear spans, crane loads, RFQ requirements, and realistic construction timelines—without misleading universal price promises.
        </p>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search cost factors, RFQ checklist, timelines, spans..."
          ...
          <div className="text-xs font-mono text-slate-500 font-semibold self-center">
            Showing {filteredArticles.length} of {COST_PLANNING_ARTICLES_DATA.length} Guides
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredArticles.map((article) => (
          <div
            key={article.id}
            onClick={() => onNavigate('resources/knowledge-hub/cost-planning', article.slug)}
                ...
                <ul className="space-y-1">
                  ...
                  {article.tableOfContents.length > 3 && (
                    <li className="text-[11px] text-sky-600 font-medium pl-4">
                      + {article.tableOfContents.length - 3} more sections
                    </li>
                  )}
                </ul>
              </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-3 max-w-2xl">
            <div className="inline-flex items-center gap-2 text-xs font-mono uppercase font-bold text-sky-700 bg-white px-2.5 py-1 rounded border border-sky-200">
              ...
              Verified Engineering Consultation
            </div>
            <h3 className="text-xl sm:text-2xl font-black text-slate-900 uppercase">
              Need a Project-Specific Structural Estimate?
            </h3>
            <p className="text-xs sm:text-sm text-slate-600 leading-relaxed">
              Every industrial building is unique. Share your dimensional grid, clear height, and crane requirements with Khodiyar Infraproject’s structural engineering team in Halol / Vadodara for an authentic, code-compliant proposal per IS 800:2007.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto flex-shrink-0">
            ...
            <button
              onClick={() => onOpenQuote('Cost & Planning RFQ Consultation')}
        ...
        <div className="pt-6 border-t border-sky-200/80 flex flex-wrap items-center justify-between gap-4 text-xs">
          ...
          <div className="flex flex-wrap gap-4 font-bold text-sky-700">
            ...
            <button 
              ...
              className="hover:text-sky-900 hover:underline cursor-pointer text-slate-700"
            >
              Steel Tonnage Estimator Tool →
            </button>
          </div>
        </div>
      </section>
